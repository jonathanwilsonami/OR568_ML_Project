...
# ------------------------- NEW: DOWNLOAD SOURCE (adds real BTS prezipped naming) -------------------------
# BTS "Reporting Carrier On-Time Performance (1987-present)" prezipped monthly files live under:
#   https://transtats.bts.gov/PREZIP/
# and are named like:
#   On_Time_Reporting_Carrier_On_Time_Performance_1987_present_2019_1.zip
# (see: BTS download page + examples of the prezipped filenames) :contentReference[oaicite:1]{index=1}
...
# Use these if you want (examples):
#   AIRPORT_PAIR = ("BWI", "ATL")  -> keeps BWI->ATL and ATL->BWI
#   ORIGIN_FILTER = ["BWI"]; DEST_FILTER = ["JFK","ATL"] -> keeps BWI->JFK and BWI->ATL only
AIRPORT_PAIR: tuple[str, str] | None = None   # e.g. ("BWI", "ATL") or ("BWI", "JFK")
ORIGIN_FILTER: list[str] | None = None        # e.g. ["BWI"]
DEST_FILTER: list[str] | None = None          # e.g. ["ATL","JFK"]
# --------------------------------------------------------------------------------------------------------
...
def process_month(year: int, month: int) -> pl.DataFrame:
    ...
    # ------------------------- MINIMAL CHANGE: use real BTS prezip filename -------------------------
    ...
    # ------------------------- NEW: build route filter expression (keeps old default behavior) ------
    # Default (your original): Origin==AIRPORT AND Dest==AIRPORT
    route_filter = (pl.col(origin_col) == AIRPORT) & (pl.col(dest_col) == AIRPORT)

    # If AIRPORT_PAIR is set: keep either direction between the two airports
    if AIRPORT_PAIR is not None:
        a, b = AIRPORT_PAIR
        route_filter = ((pl.col(origin_col) == a) & (pl.col(dest_col) == b)) | ((pl.col(origin_col) == b) & (pl.col(dest_col) == a))

    # If ORIGIN_FILTER / DEST_FILTER are set: apply them (one-way unless you set both directions yourself)
    if ORIGIN_FILTER is not None:
        route_filter = route_filter & pl.col(origin_col).is_in(ORIGIN_FILTER)
    if DEST_FILTER is not None:
        route_filter = route_filter & pl.col(dest_col).is_in(DEST_FILTER)
    # ----------------------------------------------------------------------------------------------
